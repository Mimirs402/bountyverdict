...
the402 provider credentials are written outside the repository to `~/.config/bountyverdict/the402.env` with mode `0600`, mirrored to encrypted Worker and GitHub Actions secret stores, and loaded by the distribution monitor. Fulfillment and listing contracts support all seven products, including SkillVerdict; the public provider catalog remains authoritative for assigned live service IDs. Each listing publishes the canonical typed output schema and is checked for contract drift. The current six published services remain bundled in the `BountyVerdict Agent Engineering Monthly` plan at $1.05 agent price for up to 20 combined requests until an authoritative SkillVerdict service ID is added to the plan. The signed `request.created` feed evaluates future buyer postings and bids only when the complete brief and explicit intent match one of the seven existing schemas; ambiguous, expired, subcontracted, over-tier, or unrelated work is ignored.
