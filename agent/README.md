...
The same loop emits a private agent-workflow marker only for a high-confidence new Taskmarket or MoltJobs opportunity. Taskmarket requires its official bounty record plus a successful Base receipt that binds the current seven-field `TaskCreated` task/requester/reward/expiry/stake event and exact Base-USDC escrow transfer to the pinned Taskmarket Diamond. Fresh read-only `getTask(bytes32)`, `getTaskHooks(bytes32)`, `evaluatorFor(bytes32)`, and `getTaskMetadata(bytes32)` calls against that Diamond must independently bind the same task, requester, bounty mode, reward, expiry, fee-derived net amount, current `Open` state, empty resolved hook list, zero evaluator, and creation time. It also requires public task and submission visibility, an unassigned open window, a non-owner requester, no more than three submissions, at least 5 USDC net, age no greater than 12 hours, and at least two hours remaining. MoltJobs additionally requires its open and `funded=true` feeds to agree, paired escrow identifiers, an independent public summary that agrees on job identity/economics while reporting `escrowFunded=true`, no assigned agent, and no more than three bids, plus a successful Base receipt containing the exact canonical-USDC transfer and escrow event bound to the API's 32-byte `escrowJobId`. Its 5-USDC gate conservatively treats only 95% of the posted budget as worker value. MoltJobs triggering fails closed unless `BOUNTY_MOLTJOBS_OWNER_POSTER_IDS` is explicitly `none` or the comma-separated project-owned poster UUIDs; creating a project account requires replacing `none` before the next watcher run. OpenJobs remains monitor-only because its public feed does not prove pre-acceptance USDC funding.

Algora discovery is intentionally separate and lower-frequency. `scripts/algora-scout.ts` runs the legacy `algora-pbc` and current `algora-pbc[bot]` GitHub commenter searches over a rolling 48-hour update window, deduplicates the complete bounded results, and evaluates at most 15 changed issues with the local BountyVerdict implementation. A GitHub issue enters the same guarded local-preparation workflow only when BountyVerdict returns `VIABLE` at 65 or above, an exact current Algora sponsor-page record or authenticated Algora-app listing worth at least $25, zero active claims or other hard stops, no AI-policy block, and no autonomous-execution blocker. The scout validates that `Mimirs402` is the active GitHub account, stores only bounded mode-0600 evidence in `~/.local/state/bountyverdict/algora-scout.json`, shares an exclusive marker-producer lock with the ten-minute watcher, and never comments, claims, submits, pays, or counts a listing as revenue. The three-hour user timer is defined in `ops/systemd/bountyverdict-algora-scout.{service,timer}`.
