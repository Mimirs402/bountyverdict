...
import { FREE_SELECTION_ROUTER_EXPERIMENT_ID } from "../src/task-leading-description-experiment.ts";
...
const activationPath = process.env.FREE_SELECTION_ROUTER_EXPERIMENT_ACTIVATION_FILE ||
  `${home}/.config/bountyverdict/free-selection-router-v1.activation.json`;
const checkpointPath = process.env.FREE_SELECTION_ROUTER_EXPERIMENT_STATE_FILE ||
  `${home}/.local/state/bountyverdict/experiments/free-selection-router-v1.json`;
...
const [funnelState, trustedBaseline, trustedLedger, activation, previous] = await Promise.all([
  ...
  readMeasurementExperimentCheckpoint(checkpointPath, FREE_SELECTION_ROUTER_EXPERIMENT_ID),
]);
const missing = Object.entries({ funnelState, trustedBaseline, trustedLedger, activation })
  .filter(([, value]) => value === null)
  .map(([name]) => name);
if (missing.length) throw new Error(`Free selector boundary inputs are missing: ${missing.join(", ")}`);
