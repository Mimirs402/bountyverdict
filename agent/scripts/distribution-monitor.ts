...
2. Measure whether agents discover the official MCP Registry entry, general remote-MCP handoff, mcpub registration, AgentNDX listing, MCPRepository listing, Awesome MCP Servers entry, TensorBlock MCP Index, Agentage directory, Docker MCP Registry, MCPServers.org entry, MCP.Directory entry, Cline or Kilo in-agent marketplace entry, Gemini CLI extension, or GitHub Actions intent page; then call \`tools/list\`, select a specific tool, and present payment. Watch scheduled Qt Creator and Glama propagation plus whether Agent Tools Cloud adds MCPDriftVerdict or broadens its incomplete suite metadata. Do not confuse registry presence, crawler verification, or owner checks with buyer demand.
