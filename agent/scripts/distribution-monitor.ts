...
- **OpenHands integrations registry:** source-marked no-auth Streamable HTTP catalog contract prepared for the public runtime registry; ${funnel.available ? `${Number(mcpOpenHandsIntegrations.initialize || 0)} initializations, ${Number(mcpOpenHandsIntegrations.tools_list || 0)} tool-list requests, ${Number(mcpOpenHandsIntegrations.payment_required || 0)} valid unpaid calls, ${Number(mcpOpenHandsIntegrations.payment_present || 0)} payment presentations` : "funnel unavailable"} (PR or catalog presence and aggregate events are not installs, unique agents, purchases, or revenue)
