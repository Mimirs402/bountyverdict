...
- **Goose Extensions runtime:** source attribution deployed ahead of the upstream catalog submission; ${funnel.available ? `${Number(mcpGooseExtensions.initialize || 0)} source-marked initializations, ${Number(mcpGooseExtensions.tools_list || 0)} tool-list requests, ${Number(mcpGooseExtensions.payment_required || 0)} valid unpaid calls, ${Number(mcpGooseExtensions.payment_present || 0)} payment presentations` : "funnel unavailable"} (aggregate events are not installs, unique agents, purchases, or revenue)
