...
async function acquisitionStatus(): Promise<Record<string, unknown>> {
  try {
    ...
    return {
      ...
      mcp_repository_legacy: state.mcp_repository_legacy || null,
      ...
      mcp_servers_org_legacy: state.mcp_servers_org_legacy || null,
...
- **Legacy personal directory records:** MCPRepository ${report.acquisition?.mcp_repository_legacy?.status || "unavailable"}; MCPServers.org ${report.acquisition?.mcp_servers_org_legacy?.status || "unavailable"} (observed only for migration/removal; always excluded from canonical distribution, demand, purchases, and revenue)
...
- Legacy personal directory records: MCPRepository ${report.acquisition?.mcp_repository_legacy?.status || "unavailable"}; MCPServers.org ${report.acquisition?.mcp_servers_org_legacy?.status || "unavailable"} (migration evidence only and excluded from canonical distribution, demand, purchases, and revenue)
