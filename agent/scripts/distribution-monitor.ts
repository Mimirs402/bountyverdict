...
const MCP_REGISTRY_VERSION = "1.1.26";
...
const MCP_UNKNOWN_TOOL_RECOVERY_EXPERIMENT = Object.freeze({
  id: "mcp-unknown-tool-recovery-epoch46-v1",
  recovery_release_commit: "fbcca7f31454c57a3843b7a4e005d7b80bbd60a0",
  activation_commit: "d5e5fcbe173fcc132bc7cc6f4f60834628053d2d",
  measurement_epoch_id: 46,
  target_tools_list: 25,
  eligible_prefix: Object.freeze({
    initialize: 0,
    tools_list: 0,
    protocol_error: 0,
    tool_not_found: 0,
...
const freeSelectionCatalogExperimentStateFile = process.env.FREE_SELECTION_CATALOG_EXPERIMENT_STATE_FILE ||
  `${homedir()}/.local/state/bountyverdict/experiments/mcp-free-selection-catalog-v2.json`;
const catalogFreeProofExperimentStateFile = process.env.CATALOG_FREE_PROOF_EXPERIMENT_STATE_FILE ||
  `${homedir()}/.local/state/bountyverdict/experiments/mcp-catalog-free-proof-v1.json`;
...
const catalogFreeProofActivationFile = process.env.CATALOG_FREE_PROOF_EXPERIMENT_ACTIVATION_FILE ||
...
async function mcpIntentPageStatus(): Promise<Record<string, unknown>> {
  ...
  if (body.length > 100_000 || !body.includes("<title>GitHub Actions Failure Diagnosis MCP Server</title>") ||
    ...
    !body.includes("io.github.Mimirs402/bountyverdict") ||
...
async function payanDemandStatus(): Promise<Record<string, any>> {
  ...
  if (typeof state.last_run.bid_enabled !== "boolean" ||
      typeof state.last_run.fulfill_enabled !== "boolean" ||
      state.last_run.bid_enabled !== state.last_run.fulfill_enabled) {
    throw new Error("Payan demand capture mode is malformed or inconsistent.");
  }
  const mutationsEnabled = state.last_run.bid_enabled;
  ...
  return {
    ...
    ...state.last_run,
    mode: mutationsEnabled ? "bid_and_fulfill" : "read_only_observer",
    mutations_enabled: mutationsEnabled,
    tracked_products: products,
    records,
    measurement: "exact_fit_request_observation_and_optional_bid_fulfillment_state_not_settlement_or_revenue_by_itself",
  };
}
...
async function publicDemandStatus(): Promise<Record<string, any>> {
  ...
  const zeroxwork = state.sources?.zeroxwork;
  ...
  if (state.schema_version !== 2 || state.read_only !== true || state.actions_enabled !== false ||
    ...
    !zeroxwork || typeof zeroxwork !== "object" || Array.isArray(zeroxwork) ||
  ...
  if (sourceStatus !== undefined) {
    ...
    const keys = ["moltjobs", "openjobs", "clankonomy", "zeroxwork", "taskmarket_inventory", "taskmarket_tracked"];
    let sourceErrors = 0;
  ...
  for (const [source, fields] of [[molt, [
    ...
    "public_open_tasks", "results_based_unescrowed_tasks", "social_or_physical_tasks",
    "preliminary_onchain_candidates", "chain_verified_open_tasks",
    "fresh_low_competition_candidate_count", "exact_candidate_count",
  ]], [taskmarket, [
  ...
  if (!/^\d+(?:\.\d{1,6})?$/.test(molt.nominal_open_budget_usdc) ||
    ...
    !/^\d+(?:\.\d{1,6})?$/.test(zeroxwork.public_open_value_usdc) ||
...
async function acquisitionStatus(): Promise<Record<string, unknown>> {
  try {
    ...
    return {
      ...
      askill_dedicated: state.askill_dedicated || null,
      askill_main: state.askill_main || null,
      ...
      openhands_integrations: state.openhands_integrations || null,
      goose_extensions: state.goose_extensions || null,
      ...
      mcp_marketplace: state.mcp_marketplace || null,
      mcp_market_com: state.mcp_market_com || null,
...
async function funnelStatus(
  ...
  previousCatalogFreeProofExperiment: unknown,
): Promise<Record<string, unknown>> {
  try {
    const readDescriptionActivation = async (path: string, label: string): Promise<unknown> => {
      ...
      try {
        ...
        if (!metadata.isFile() || expectedUid < 0 || metadata.uid !== expectedUid ||
          ...
          throw new Error(`${label} activation must be a bounded regular owner-owned file with mode 0600.`);
        }
    ...
    const taskLeadingDescriptionActivation = await readDescriptionActivation(
      taskLeadingDescriptionActivationFile,
      "Task-leading description",
    );
    ...
    const catalogFreeProofActivation = await readDescriptionActivation(
      catalogFreeProofActivationFile,
      "Catalog free proof",
    );
    ...
    const mcpPreviewCopyExperiment = {
      ...
      ...updateSelectionPreviewExperiment({
        id: MCP_PREVIEW_COPY_ROLLOUT.id,
        observedAt: new Date().toISOString(),
        targetToolsList: MCP_PREVIEW_COPY_ROLLOUT.target_tools_list,
        resumeEpochId: MCP_PREVIEW_COPY_ROLLOUT.resume_epoch_id,
        eligiblePrefix: MCP_PREVIEW_COPY_ROLLOUT.eligible_prefix_before_audited_drain,
        rawDelta: mcpPreviewCopyDelta,
    ...
    const mcpUnknownToolRecoveryExperiment = {
      ...MCP_UNKNOWN_TOOL_RECOVERY_EXPERIMENT,
      ...updateUnknownToolRecoveryExperiment({
        id: MCP_UNKNOWN_TOOL_RECOVERY_EXPERIMENT.id,
        observedAt: new Date().toISOString(),
        targetToolsList: MCP_UNKNOWN_TOOL_RECOVERY_EXPERIMENT.target_tools_list,
        measurementEpochId: MCP_UNKNOWN_TOOL_RECOVERY_EXPERIMENT.measurement_epoch_id,
        currentEpochId: Number(trustedBaseline.epoch_id || 1),
        measurementEligible,
        cleanEpochDelta: effectiveTrustedMcp?.buyer_candidate_totals || null,
    ...
    const mcpAgentQuestionDescriptionExperiment = updateTaskLeadingDescriptionExperiment({
      experimentId: AGENT_QUESTION_DESCRIPTION_EXPERIMENT_ID,
      observedAt: new Date().toISOString(),
      activation: agentQuestionDescriptionActivation,
      currentEpochId: Number(trustedBaseline.epoch_id || 1),
      measurementEligible,
      cleanEpochDelta: effectiveTrustedMcp?.buyer_candidate_totals || null,
      trustedBaselineInitializedAt: trustedBaseline.initialized_at,
    ...
    const mcpCatalogFreeProofExperiment = updateTaskLeadingDescriptionExperiment({
      experimentId: CATALOG_FREE_PROOF_EXPERIMENT_ID,
      observedAt: new Date().toISOString(),
      activation: catalogFreeProofActivation,
      currentEpochId: Number(trustedBaseline.epoch_id || 1),
      measurementEligible,
      cleanEpochDelta: effectiveTrustedMcp?.buyer_candidate_totals || null,
      trustedBaselineInitializedAt: trustedBaseline.initialized_at,
      trustedRotation: epochRotation,
      previous: previousCatalogFreeProofExperiment,
    });
    ...
    return {
      ...
      trusted_mcp_protocol_error_kinds: effectiveTrustedMcp?.protocol_error_kinds || {},
      trusted_mcp_protocol_error_kinds_by_channel: effectiveTrustedMcp?.protocol_error_kinds_by_channel || {},
      ...
      mcp_catalog_free_proof_experiment: mcpCatalogFreeProofExperiment,
...
function renderMonitorNote(report: Record<string, any>): string {
  ...
  const zeroxWorkDemand = publicDemand.zeroxwork || {};
  ...
  const trustedMcpProtocolErrorKinds = funnel.trusted_mcp_protocol_error_kinds || {};
  ...
  const mcpCatalogFreeProofExperiment = funnel.mcp_catalog_free_proof_experiment || {};
  const mcpCatalogFreeProofDelta = mcpCatalogFreeProofExperiment.eligible_delta || {};
  ...
  const mcpCursorDeeplink = funnel.mcp_by_channel?.cursor_deeplink || {};
  const mcpVsCodeDeeplink = funnel.mcp_by_channel?.vscode_deeplink || {};
  const mcpOpenHandsIntegrations = funnel.mcp_by_channel?.openhands_integrations || {};
  const mcpGooseExtensions = funnel.mcp_by_channel?.goose_extensions || {};
  ...
  const githubPrWorkflowTelemetryAvailable = Number(githubPrMonitoring.checked || 0) >= 12 &&
  ...
  const x402Arena = report.acquisition?.x402arena || {};
...
- **Clawlancer outstanding transaction:** ${clawlancer.available ? `${String(clawlancer.status || "unknown").toUpperCase()}; ${clawlancer.transaction?.fundingTxHash ? "funding transaction reported" : "no funding transaction"}; ${clawlancer.onchain_evidence?.verified ? "release verified on Base and recognized" : "no onchain-verified release, not revenue"}; observed action ${clawlancer.action || "unknown"}` : `unavailable (${clawlancer.error || "state not captured"})`} (read-only reconciliation only; delivery remains disabled)
...
- **MCP zero-argument catalog experiment:** ${funnel.available ? `${mcpFreeSelectionCatalogExperiment.status || "unavailable"}; exact fresh epoch ${mcpFreeSelectionCatalogExperiment.measurement_epoch_id ?? "pending activation"}; eligible zero-prefix delta ${Number(mcpFreeSelectionCatalogDelta.initialize || 0)} initialize / ${Number(mcpFreeSelectionCatalogDelta.tools_list || 0)} tools/list / ${Number(mcpFreeSelectionCatalogDelta.selection_preview || 0)} free catalog calls / ${Number(mcpFreeSelectionCatalogDelta.validation_error || 0)} invalid input / ${Number(mcpFreeSelectionCatalogDelta.payment_required || 0)} valid unpaid / ${Number(mcpFreeSelectionCatalogDelta.payment_present || 0)} payment presented / ${Number(mcpFreeSelectionCatalogDelta.paid_success || 0)} paid success / ${Number(mcpFreeSelectionCatalogDelta.paid_error || 0)} paid error; ${Number(mcpFreeSelectionCatalogExperiment.remaining_eligible_tools_list ?? 25)} eligible lists remaining; decision ${mcpFreeSelectionCatalogExperiment.decision || "pending"}` : "unavailable"} (v2 excludes the invalidated report-audit cohort and measures v1.1.15 from a fresh zero prefix; only local telemetry checkpointing is allowed before the boundary; a free catalog call is not payment intent, a purchase, or revenue)
- **MCP catalog free-proof experiment:** ${funnel.available ? `${mcpCatalogFreeProofExperiment.status || "unavailable"}; exact fresh epoch ${mcpCatalogFreeProofExperiment.measurement_epoch_id ?? "pending activation"}; eligible zero-prefix delta ${Number(mcpCatalogFreeProofDelta.initialize || 0)} initialize / ${Number(mcpCatalogFreeProofDelta.tools_list || 0)} tools/list / ${Number(mcpCatalogFreeProofDelta.selection_preview || 0)} free selector calls / ${Number(mcpCatalogFreeProofDelta.validation_error || 0)} invalid input / ${Number(mcpCatalogFreeProofDelta.payment_required || 0)} valid unpaid / ${Number(mcpCatalogFreeProofDelta.payment_present || 0)} payment presented / ${Number(mcpCatalogFreeProofDelta.paid_success || 0)} paid success / ${Number(mcpCatalogFreeProofDelta.paid_error || 0)} paid error; ${Number(mcpCatalogFreeProofExperiment.remaining_eligible_tools_list ?? 25)} eligible lists remaining; decision ${mcpCatalogFreeProofExperiment.decision || "pending"}` : "unavailable"} (each paid tools/list description exposes its canonical free representative result and exact authorization cap; only fresh non-owner aggregate activity is eligible, and no causal same-agent journey is inferred)
...
- **Cursor direct MCP workflow:** official one-click MCP deeplink published; Marketplace publication is intentionally excluded because its Publisher Terms prohibit direct or indirect user fees, which conflicts with x402 calls; ${funnel.available ? `${Number(mcpCursorDeeplink.initialize || 0)} deeplink-source initializations, ${Number(mcpCursorDeeplink.tools_list || 0)} deeplink tool-list requests` : "funnel unavailable"} (source-marked aggregates are not clicks, installs, unique agents, purchases, or revenue)
- **VS Code direct MCP workflow:** official source-marked one-click MCP deeplink published alongside the Registry-backed gallery entry; ${funnel.available ? `${Number(mcpVsCodeDeeplink.initialize || 0)} deeplink-source initializations, ${Number(mcpVsCodeDeeplink.tools_list || 0)} deeplink tool-list requests` : "funnel unavailable"} (source-marked aggregates are not clicks, installs, unique agents, purchases, or revenue)
- **OpenHands integrations registry:** ${report.acquisition?.openhands_integrations?.status || "pr_open_pending_directory_refresh"}; PR [#416](${report.acquisition?.openhands_integrations?.url || "https://github.com/OpenHands/extensions/pull/416"}); exact no-auth Streamable HTTP contract ${report.acquisition?.openhands_integrations?.contract_verified ? "verified live" : "pending merge"}; ${funnel.available ? `${Number(mcpOpenHandsIntegrations.initialize || 0)} initializations, ${Number(mcpOpenHandsIntegrations.tools_list || 0)} tool-list requests, ${Number(mcpOpenHandsIntegrations.payment_required || 0)} valid unpaid calls, ${Number(mcpOpenHandsIntegrations.payment_present || 0)} payment presentations` : "funnel unavailable"} (PR or catalog presence and aggregate events are not installs, unique agents, purchases, or revenue)
- **goose Extensions runtime:** ${report.acquisition?.goose_extensions?.status || "pr_open_pending_directory_refresh"}; PR [#10625](${report.acquisition?.goose_extensions?.url || "https://github.com/aaif-goose/goose/pull/10625"}); exact keyless Streamable HTTP contract ${report.acquisition?.goose_extensions?.contract_verified ? "verified live" : "pending merge"}; ${funnel.available ? `${Number(mcpGooseExtensions.initialize || 0)} source-marked initializations, ${Number(mcpGooseExtensions.tools_list || 0)} tool-list requests, ${Number(mcpGooseExtensions.payment_required || 0)} valid unpaid calls, ${Number(mcpGooseExtensions.payment_present || 0)} payment presentations` : "funnel unavailable"} (PR or catalog presence and aggregate events are not installs, unique agents, purchases, or revenue)
- **MCP Marketplace:** ${report.acquisition?.mcp_marketplace?.status || "unavailable"}; ${Number(report.acquisition?.mcp_marketplace?.install_count || 0)} anonymous installs; security ${report.acquisition?.mcp_marketplace?.security_score ?? "unavailable"}/10; pricing disclosure ${report.acquisition?.mcp_marketplace?.pricing_disclosure_state || "unavailable"}; ownership ${report.acquisition?.mcp_marketplace?.claimed ? "claimed" : "unclaimed"}; agent search ${report.acquisition?.mcp_marketplace?.buyer_query_benchmark?.status || "unavailable"} with ${Number(report.acquisition?.mcp_marketplace?.buyer_query_benchmark?.found_queries || 0)} / ${Number(report.acquisition?.mcp_marketplace?.buyer_query_benchmark?.query_count || 0)} fixed unbranded tasks found (Registry import, owner-run searches, and anonymous installs are acquisition signals, not marketplace query volume, unique users, valid calls, purchases, or revenue)
- **MCP Market:** ${report.acquisition?.mcp_market_com?.status || "unavailable"}; paid x402 copy ${report.acquisition?.mcp_market_com?.paid_x402_disclosed ? "verified" : "unverified"}; business owner ${report.acquisition?.mcp_market_com?.business_owner_verified ? "verified" : "unverified"}; canonical product link ${report.acquisition?.mcp_market_com?.canonical_product_link_verified ? "verified" : "unverified"} (public listing and copy contract only; the marketplace exposes no bounded public impression, install, or call counter, so none is inferred)
...
- **Canonical askill skill suite:** ${report.acquisition?.askill_main?.listed ? `${Number(report.acquisition.askill_main.listed_skills || 0)} / 7 exact task skills listed; ${Number(report.acquisition.askill_main.favorites || 0)} public favorites; unbranded retrieval ${Number(report.acquisition.askill_main.buyer_query_benchmark?.found_queries || 0)} / ${Number(report.acquisition.askill_main.buyer_query_benchmark?.query_count || ASKILL_BUYER_QUERIES.length)} queries found and ${Number(report.acquisition.askill_main.buyer_query_benchmark?.top_three_queries || 0)} top-three` : report.acquisition?.askill_main?.status || "awaiting audited catalog refresh"} (one canonical repository submission; listing and owner-run rank evidence are never impressions, installs, calls, purchases, or revenue)
...
- **VS Code direct install:** official source-marked MCP deeplink is public in the canonical guide; aggregate source events are ${funnel.available ? `${Number(mcpVsCodeDeeplink.initialize || 0)} initialize / ${Number(mcpVsCodeDeeplink.tools_list || 0)} tools/list` : "unavailable"}, never clicks, installs, unique agents, purchases, or revenue
...
- Dedicated askill adapter: ${report.acquisition?.askill_dedicated?.listed_skills ?? 0} / 1 exact skill listed; ${report.acquisition?.askill_dedicated?.status || "unavailable"}; ${Number(report.acquisition?.askill_dedicated?.favorites || 0)} public favorites; fixed unbranded retrieval ${Number(report.acquisition?.askill_dedicated?.buyer_query_benchmark?.found_queries || 0)} / ${Number(report.acquisition?.askill_dedicated?.buyer_query_benchmark?.query_count || ASKILL_BUYER_QUERIES.length)} found and ${Number(report.acquisition?.askill_dedicated?.buyer_query_benchmark?.top_three_queries || 0)} top-three (${report.acquisition?.askill_dedicated?.url || "https://askill.sh/skills/703722"}; dedicated-account placement is measured separately from the frozen legacy experiment and is never purchase or revenue proof)
- Canonical askill skill suite: ${report.acquisition?.askill_main?.listed_skills ?? 0} / 7 exact skills listed; ${report.acquisition?.askill_main?.status || "unavailable"}; ${Number(report.acquisition?.askill_main?.favorites || 0)} public favorites; fixed unbranded retrieval ${Number(report.acquisition?.askill_main?.buyer_query_benchmark?.found_queries || 0)} / ${Number(report.acquisition?.askill_main?.buyer_query_benchmark?.query_count || ASKILL_BUYER_QUERIES.length)} found and ${Number(report.acquisition?.askill_main?.buyer_query_benchmark?.top_three_queries || 0)} top-three (${report.acquisition?.askill_main?.url || "https://askill.sh/"}; exact listing and owner-run ranks are not search volume, impressions, installs, calls, purchases, or revenue)
...
- PayanAgent offers: ${report.marketplaces?.payan?.offer_count ?? "unavailable"} / ${report.marketplaces?.payan?.expected_offer_count ?? PAYAN_OFFERS.length} (Base x402 proxy; SkillVerdict ${report.marketplaces?.payan?.skillverdict_listed ? "listed" : "not listed"}); exact-fit demand ${report.marketplaces?.payan?.demand_capture?.healthy ? report.marketplaces.payan.demand_capture.mode === "read_only_observer" ? "read-only observer healthy" : "bid/fulfillment automation healthy" : "unavailable"}
...
- Trusted MCP protocol-error reasons: ${funnel.available && trustedMcpAvailable ? `${Number(trustedMcpProtocolErrorKinds.unsupported_protocol || 0)} unsupported protocol, ${Number(trustedMcpProtocolErrorKinds.malformed_tool_call || 0)} malformed tool call, ${Number(trustedMcpProtocolErrorKinds.free_selector_schema_rejected || 0)} free-selector schema rejected, ${Number(trustedMcpProtocolErrorKinds.legacy_unclassified || 0)} legacy unclassified` : "unavailable"} (privacy-safe aggregate categories only; same buyer-channel exclusions as the trusted MCP funnel; not unique agents, linked sessions, purchases, or revenue)
...
- Cursor deeplink source capture: ${funnel.available ? `${Number(mcpCursorDeeplink.initialize || 0)} initialize, ${Number(mcpCursorDeeplink.tools_list || 0)} tools/list, ${Number(mcpCursorDeeplink.validation_error || 0)} invalid calls, ${Number(mcpCursorDeeplink.payment_required || 0)} unpaid valid calls, ${Number(mcpCursorDeeplink.payment_present || 0)} payment presentations, ${Number(mcpCursorDeeplink.paid_success || 0)} paid successes` : "unavailable"} (allowlisted aggregate marker only; not a click, install, identity, unique agent, purchase, or revenue)
- VS Code deeplink source capture: ${funnel.available ? `${Number(mcpVsCodeDeeplink.initialize || 0)} initialize, ${Number(mcpVsCodeDeeplink.tools_list || 0)} tools/list, ${Number(mcpVsCodeDeeplink.validation_error || 0)} invalid calls, ${Number(mcpVsCodeDeeplink.payment_required || 0)} unpaid valid calls, ${Number(mcpVsCodeDeeplink.payment_present || 0)} payment presentations, ${Number(mcpVsCodeDeeplink.paid_success || 0)} paid successes` : "unavailable"} (allowlisted aggregate marker only; not a click, install, identity, unique agent, purchase, or revenue)
- OpenHands integrations source capture: ${funnel.available ? `${Number(mcpOpenHandsIntegrations.initialize || 0)} initialize, ${Number(mcpOpenHandsIntegrations.tools_list || 0)} tools/list, ${Number(mcpOpenHandsIntegrations.validation_error || 0)} invalid calls, ${Number(mcpOpenHandsIntegrations.payment_required || 0)} unpaid valid calls, ${Number(mcpOpenHandsIntegrations.payment_present || 0)} payment presentations, ${Number(mcpOpenHandsIntegrations.paid_success || 0)} paid successes` : "unavailable"} (allowlisted aggregate marker only; not a catalog impression, install, identity, unique agent, purchase, or revenue)
...
const persistedAgentQuestionDescriptionExperiment = await readMeasurementExperimentCheckpoint(
  agentQuestionDescriptionExperimentStateFile,
  AGENT_QUESTION_DESCRIPTION_EXPERIMENT_ID,
);
...
const persistedCatalogFreeProofExperiment = await readMeasurementExperimentCheckpoint(
  catalogFreeProofExperimentStateFile,
  CATALOG_FREE_PROOF_EXPERIMENT_ID,
);
const previousCatalogFreeProofExperiment = persistedCatalogFreeProofExperiment ||
  previousReport.funnel?.mcp_catalog_free_proof_experiment || null;
...
try {
  acquisition = {
    ...acquisition,
    x402arena: await x402ArenaStatus(),
  };
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
  acquisition = { ...acquisition, x402arena: { listed: false, checked_at: checkedAt, error: message } };
  errors.push(`x402 Arena: ${message}`);
}
...
funnel = await funnelStatus(
  ...
  previousCatalogFreeProofExperiment,
);
...
const currentAgentQuestionDescriptionExperiment = funnel.mcp_agent_question_description_experiment;
if (!currentAgentQuestionDescriptionExperiment || typeof currentAgentQuestionDescriptionExperiment !== "object" ||
  Array.isArray(currentAgentQuestionDescriptionExperiment)) {
  throw new Error("Agent-question description experiment state is missing from the funnel report.");
}
await writeMeasurementExperimentCheckpoint(
  agentQuestionDescriptionExperimentStateFile,
  AGENT_QUESTION_DESCRIPTION_EXPERIMENT_ID,
...
const currentCatalogFreeProofExperiment = funnel.mcp_catalog_free_proof_experiment;
if (!currentCatalogFreeProofExperiment || typeof currentCatalogFreeProofExperiment !== "object" ||
  Array.isArray(currentCatalogFreeProofExperiment)) {
  throw new Error("Catalog free-proof experiment state is missing from the funnel report.");
}
await writeMeasurementExperimentCheckpoint(
  catalogFreeProofExperimentStateFile,
  CATALOG_FREE_PROOF_EXPERIMENT_ID,
  checkedAt,
  currentCatalogFreeProofExperiment as Record<string, unknown>,
);
