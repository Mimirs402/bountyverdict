...
const unsignedPreviewGuidance = (price: string) => `A first unsigned call with real input cannot charge; it returns a free payment quote and selection summary. Only an authorized signed retry costs ${price} USDC on Base.`;

const TOOL_DESCRIPTIONS: Record<ToolName, string> = {
  check_github_bounty: `Assess whether one public GitHub bounty is worth pursuing before you code using current public evidence, with an evidence-linked AVOID, CAUTION, or VIABLE claimability-risk verdict. For 2-10 candidates use rank_github_bounties. Read-only; never invent arguments. ${unsignedPreviewGuidance(PRODUCT_CATALOG.single.priceUsd)} Sample: https://bountyverdict-agent-production.mimirslab.workers.dev/api/sample.`,
  rank_github_bounties: `Rank 2-10 public GitHub bounty issues and identify the strongest non-AVOID candidate—or recommend none—with full verdicts, cited evidence, and partial-failure handling. For one issue use check_github_bounty. Read-only; never invent arguments. ${unsignedPreviewGuidance(PRODUCT_CATALOG.portfolio.priceUsd)} Sample: https://bountyverdict-agent-production.mimirslab.workers.dev/api/portfolio/sample.`,
  audit_agent_harness: `Audit a public GitHub repository's AGENTS.md, CLAUDE.md, and related agent-instruction stack at an immutable commit before assigning a coding agent. This does not diagnose CI. Read-only; never invent arguments. ${unsignedPreviewGuidance(PRODUCT_CATALOG.harness.priceUsd)} Sample: https://bountyverdict-agent-production.mimirslab.workers.dev/api/harness/sample.`,
  diagnose_github_actions_run: `Diagnose why one public GitHub Actions run failed, with redacted evidence and concrete next actions. For retry-once versus fix use classify_github_actions_flake. Read-only; never invent arguments. ${unsignedPreviewGuidance(PRODUCT_CATALOG.run.priceUsd)} Sample: https://bountyverdict-agent-production.mimirslab.workers.dev/api/run/sample.`,
  classify_github_actions_flake: `Decide whether one completed failed GitHub Actions run should be retried once or fixed, using attempt and historical fingerprints. For root cause use diagnose_github_actions_run. Read-only; never invent arguments. ${unsignedPreviewGuidance(PRODUCT_CATALOG.flake.priceUsd)} Sample: https://bountyverdict-agent-production.mimirslab.workers.dev/api/flake/sample.`,
  check_mcp_tool_drift: `Detect breaking schema or safety-hint changes in the documented comparison subset by comparing caller-asserted complete baseline and current MCP tools/list snapshots. It never fetches or invokes tools. Read-only; never invent or truncate snapshot arguments. ${unsignedPreviewGuidance(PRODUCT_CATALOG.mcpdrift.priceUsd)} Workflow: https://mimirs402.github.io/bountyverdict/skills/check-mcp-tool-drift/SKILL.md.`,
};
