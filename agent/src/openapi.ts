...
export function createOpenApi(
  ...
  const completeDocument = {
    ...
    info: {
      ...
      "x-catalog-scope": "This canonical machine catalog contains exactly seven paid POST decision operations. Free samples remain live at each operation's x-free-sample URL; legacy query transports remain live but are intentionally excluded from the canonical operation catalog.",
  ...
  const paths = {
    "/api/bounty-preflight": { post: completeDocument.paths["/api/bounty-preflight"].post },
    "/api/portfolio": { post: completeDocument.paths["/api/portfolio"].post },
    "/api/repository-agent-instructions-audit": {
      post: completeDocument.paths["/api/repository-agent-instructions-audit"].post,
    },
    "/api/skill": { post: completeDocument.paths["/api/skill"].post },
    "/api/github-actions-run-diagnosis": {
      post: completeDocument.paths["/api/github-actions-run-diagnosis"].post,
    },
    "/api/github-actions-flake-retry-gate": {
      post: completeDocument.paths["/api/github-actions-flake-retry-gate"].post,
    },
    "/api/mcp-drift": { post: completeDocument.paths["/api/mcp-drift"].post },
  };
  return { ...completeDocument, paths };
}
...
- Remote MCP server: ${origin}/mcp (MCP 2025-11-25 Streamable HTTP; one free selector plus six paid read-only tools; SkillVerdict excluded)
...
BountyVerdict checks the first and newest bounded comment windows, up to four bounded timeline pages, authenticated GitHub App bounty listings and claim-state comments, Lightning Bounties rewards bound by immutable GitHub issue ID with exact per-reward lock timestamps reconciled against aggregate totals so locked and reclaimable sats stay separate, exact BountyHub records and pledge-level prepaid-versus-promised funding, non-cash reward denomination drift, one explicitly linked external GitHub source issue, rewarded labels, competing and failed PRs, attempt swarms, issue locks, repository state, explicit maintainer rejection, reward-withdrawal language, conventional contribution-document paths, explicit repository-policy bans on automated bounty claims or assignment requests, and authoritative repository-wide bounty-policy issues for AI-work bans, fake or unpaid rewards, and sensitive disclosure demands. It does not automatically access Algora's web service. Bounded policy-page truncation is explicit and prevents a VIABLE verdict. Platform listings still require creator approval and do not guarantee payout. Important signals include public evidence URLs and coverage counts.
