...
BountyVerdict checks the first and newest bounded comment windows, up to four bounded timeline pages, current Algora status tables, transfer-safe official Algora sponsor records, Lightning Bounties rewards bound by immutable GitHub issue ID with locked and reclaimable sats kept separate, exact BountyHub records and pledge-level prepaid-versus-promised funding, non-cash reward denomination drift, one explicitly linked external GitHub source issue, rewarded labels, competing and failed PRs, attempt swarms, issue locks, repository state, explicit maintainer rejection, reward-withdrawal language, conventional contribution-document paths, and authoritative repository-wide bounty-policy issues for AI-work bans, fake or unpaid rewards, and sensitive disclosure demands. Bounded policy-page truncation is explicit and prevents a VIABLE verdict. Platform listings still require creator approval and do not guarantee payout. Important signals include public evidence URLs and coverage counts.
