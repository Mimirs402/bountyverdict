...
replaced with real canonical input. It sets `payment_required: false` and
returns no verdict.
...
6. Preserve the existing schema budgets: at most 2,048 bytes per output schema and 12,000 bytes total. The current seven-tool catalog measures 11,957 bytes total with a 2,021-byte maximum.
