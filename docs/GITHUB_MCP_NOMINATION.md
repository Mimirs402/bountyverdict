...
GitHub's official publication route requires one email to `partnerships@github.com` after the server is present in the OSS MCP Registry. The release gate must first verify `io.github.Mimirs402/bountyverdict@1.1.11` in that registry before this nomination is sent.
