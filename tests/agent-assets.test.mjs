...
test("agent manifest is honest and links inspectable products", async () => {
  ...
  assert.equal(manifest.release_version, "1.1.24");
  assert.match(manifest.release_url, /\/releases\/tag\/v1\.1\.24$/);
  ...
  assert.equal(manifest.mcp.server_version, "1.1.24");
...
test("agent landing page exposes all seven self-serve products", async () => {
  ...
  assert.match(page, /gh skill preview Mimirs402\/bountyverdict route-github-agent-checks@v1\.1\.24/);
  assert.match(page, /gh skill install Mimirs402\/bountyverdict route-github-agent-checks --pin v1\.1\.24/);
