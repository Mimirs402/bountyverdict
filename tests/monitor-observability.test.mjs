...
test("question-shaped MCP selection copy gets an isolated v7 checkpoint after the Agent Finder drain", async () => {
  const distribution = await readFile(distributionUrl, "utf8");
  const activationTemplate = JSON.parse(await readFile(new URL(
  ...
  assert.match(distribution, /experiments\/mcp-agent-question-descriptions-v7\.json/);
...
test("directory monitoring tracks ToolHive review and exact in-agent remote contract without claiming demand", async () => {
  ...
  assert.match(parser, /TOOLHIVE_SERVER_VERSION = "1\.1\.26"/);
});
...
test("fresh high-confidence marketplace markers launch one deduplicated guarded agent workflow", async () => {
  ...
  assert.match(service, /StateDirectory=bountyverdict/);
  assert.match(service, /BindReadOnlyPaths=.*%h\/Projects\/sandbox\/bountyverdict.*%h\/\.codex\/auth\.json/);
  ...
  assert.match(workflow, /shell_environment_policy=\{inherit="none"/);
  assert.match(workflow, /sandbox_workspace_write\.network_access=true/);
  assert.match(workflow, /features\.network_proxy\.enabled=true/);
  assert.match(workflow, /features\.network_proxy\.domains=/);
...
test("the normal distribution timer is report-only and retains explicit accounting identity", async () => {
  ...
  assert.match(service, /Environment=TRACKED_COSTS_USDC=1\.057/);
